...
- [ ] 1. Set up project structure and shared types
  - [ ] 1.1 Create package layout and shared HTTP result types
...
- [ ] 2. Implement snapshot validation
  - [ ] 2.1 Implement UTC timestamp normalization
  ...
  - [ ] 2.2 Implement `ValidationResult` and `validate_snapshot`
  ...
  - [ ]* 2.3 Write property test for validation acceptance
  ...
  - [ ]* 2.4 Write property test for rejection field-naming
...
- [ ] 3. Implement the Statistics_Store adapter
  - [ ] 3.1 Define `StatisticsStore` protocol and `PutOutcome`
  ...
  - [ ] 3.2 Implement in-memory `StatisticsStore` fake with `put_if_newer`
  ...
  - [ ]* 3.3 Write property test for latest-instant convergence
    - **Property 6: The current snapshot is the latest instant, regardless of order**
    - **Validates: Requirements 4.2, 4.3, 4.6**

  - [ ]* 3.4 Write property test for per-station independence
  ...
  - [ ]* 3.5 Write property test for idempotent resubmission
    - **Property 10: Resubmitting a stored snapshot is idempotent**
    - **Validates: Requirements 8.8**

  - [ ] 3.6 Implement DynamoDB-backed `StatisticsStore`
...
- [ ] 4. Implement the Snapshot (POST) handler
  - [ ] 4.1 Implement `handle_snapshot`
  ...
  - [ ]* 4.2 Write property test for accept-and-store
    - **Property 1: Valid snapshots are accepted and stored**
    - **Validates: Requirements 1.2, 1.4**

  - [ ]* 4.3 Write property test for extra-field stripping
    - **Property 5: Extra fields are stripped and do not affect acceptance**
    - **Validates: Requirements 2.7**

  - [ ]* 4.4 Write property test for generic 5xx bodies
  ...
  - [ ]* 4.5 Write unit tests for malformed-JSON and oversize bodies
    - Unparseable body → `400`; body > 16KB → `413` with max-body-size message and nothing stored
    - _Requirements: 2.5, 1.6_

- [ ] 5. Implement the Metrics (GET) handler
  - [ ] 5.1 Implement `get_station` and `list_stations`
...
- [ ] 6. Implement logging and observability
  ...
  - [ ]* 6.2 Write property test for log/response hygiene
  ...
  - [ ]* 6.3 Write property test for one-complete-log-entry-per-request
    - **Property 19: Every handled request produces one complete log entry**
    - **Validates: Requirements 10.1**

  - [ ]* 6.4 Write unit test for logging-failure resilience
    - When the log emit raises, the request still returns its intended status
    - _Requirements: 10.5_

- [ ] 7. Checkpoint - server-side logic
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 8. Wire the API Gateway and Lambda entry points
  - [ ] 8.1 Define API Gateway resources, methods, and policies (IaC)
  ...
  - [ ] 8.2 Implement the Lambda event adapter wiring handlers to the gateway
...
- [ ] 9. Implement client models and the Pending_Snapshot_Buffer
  - [ ] 9.1 Implement client data models
    - `Snapshot`, `ClientConfig` (credential never logged), and the `SubmitOutcome` enum (`PUBLISHED`/`DUPLICATE`/`DISCARDED`/`RETRY`)
    - _Requirements: 7.1, 7.2_

  - [ ] 9.2 Implement `PendingSnapshotBuffer`
  ...
  - [ ]* 9.3 Write property test for the bounded buffer
    - **Property 12: The pending buffer is bounded and evicts the oldest**
    - **Validates: Requirements 8.4**

- [ ] 10. Implement the backoff scheduler
  - [ ] 10.1 Implement `_next_backoff`
  ...
  - [ ]* 10.3 Write unit test for concrete backoff values
    - Assert the sequence 2, 4, 8, 16, 32, 60, 60 and that no fixed max-attempt cap exists
    - _Requirements: 8.2, 8.3_

- [ ] 11. Implement the LaneSight_Client submission and retry logic
  - [ ] 11.1 Implement enqueue, configuration check, and submission
  ...
  - [ ] 11.2 Implement outcome handling, drain ordering, and retry scheduling
...
- [ ] 13. Implement the Connectivity_Notification
  ...
  - [ ]* 13.2 Write property test for notification state derivation
    - **Property 17: Notification state derives from the latest outcome and buffer**
    - **Validates: Requirements 9.1, 9.2, 9.5, 9.7**

  - [ ] 13.3 Implement the GUI Connectivity_Notification surface
  ...
  - [ ]* 13.4 Write UI tests for the notification surface
...
- [ ] 14. Integration, smoke, and load tests
  ...
  - [ ]* 14.7 Write smoke tests for routing and TLS policy
